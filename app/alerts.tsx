...
import { View, Text, Image, TextInput, TouchableOpacity, StyleSheet, FlatList, TouchableWithoutFeedback, Modal, Switch, Platform } from 'react-native';
...
const AlertSettingsScreen: React.FC = () => {
    ...
    const updateInterval = (key: 'water' | 'move', value: number) => {
      ...
      const alertTitles: { key: keyof AlertsState; title: string; isReminder: boolean }[] = [
        ...
        <View style={styles.container}>
            ...
            {alertTitles.map(({ key, title, isReminder }) => (
                <AlertItem
                    ...
                    onIntervalChange={(value) => updateInterval(key as 'water' | 'move', value)}
                    onTimeChange={(field, value) => updateTime(key as 'sleep' | 'wakeUp' | 'lunch' | 'snack' | 'dinner', field, value)}
                />
            ))}
        </View>
    );
